...
class GitHubService
{
    ...
    public function deleteLabel(string $owner, string $repo, string $label): int
    {
        $isInUse = $this->checkIfLabelIsInUse($owner, $repo, $label);
        if ($isInUse) {
            Log::error('Label is in use, skipping deletion');
        ...
        try {
            ...
            Log::error('Failed to delete label', ['exception' => $e]);
    ...
    private function checkIfLabelIsInUse(string $owner, string $repo, string $label): bool
    {
        $restClient = $this->getRestClient();
        $url = "https://api.github.com/search/issues?q=repo:$owner/$repo+label:\"$label\"";
        try {
            $response = $restClient->get($url);
            $json = json_decode($response->getBody()->getContents(), true, 512, JSON_THROW_ON_ERROR);
